...
#define STOPTIMES_LINES 9739513
#define STOPS_LINES 34594
...
namespace gtfs
{
    ...
    inline void loading(int& counter, int& perc, int lines,int& check)
    {
        counter++;
        if (lines < 100)
            lines = 100;
        if (((counter / (lines/ 100)) > perc) && (check == 0))
        {
            if (perc >= 99)
            {
    ...
    inline void write_agency_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = { "agency_id",       "agency_name", "agency_url",
                                           "agency_timezone", "agency_lang", "agency_phone",
                                           "agency_fare_url" };
    ...
    inline void write_routes_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = {
            "route_id",         "agency_id",        "route_short_name",  "route_long_name",
    ...
    inline void write_shapes_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = { "shape_id", "shape_pt_lat", "shape_pt_lon",
                                           "shape_pt_sequence" };
        write_joined(out, std::move(fields));
    ...
    inline void write_trips_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = {
    ...
    inline void write_stops_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = { "stop_id",        "stop_code",     "stop_name",
                                           "stop_desc",      "stop_lat",      "stop_lon",
                                            "location_type",
    ...
    inline void write_stop_times_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = {
            "trip_id",           "arrival_time",        "departure_time",      "stop_id",
    ...
    inline void write_calendar_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = { "service_id", "sunday", "monday",   "tuesday", "wednesday",  "thursday",
    ...
    inline void write_translations_header_ISR(std::ofstream& out)
    {
        std::vector<std::string> fields = { "table_name", "field_name",    "language",   "translation",
                                           "record_id",  "record_sub_id", "field_value" };
        write_joined(out, std::move(fields));
    ...
    public:
        ...
        inline void setTime(int hh, int mm, int ss);
    ...
    inline void Time::setTime(int hh, int mm, int ss)
    {
        this->hh = hh;
        this->mm = mm;
        this->ss = ss;
    ...
    {
        size_t tempV = lhs.get_total_seconds() - rhs.get_total_seconds();
        Time temp(tempV);
        return temp;
    }
    inline Time operator+(const Time& lhs, const Time& rhs)
    {
        Time temp(lhs.get_total_seconds() + rhs.get_total_seconds());
        return temp;
    }
    ...
    inline Time::Time(size_t seconds)
        ...
        hh(seconds / 3600), mm((seconds % 3600) / 60), ss((seconds % 3600)%60)
    {
        set_raw_time();
    }
    ...
    public:
        ...
        inline std::string getYYYY();
        inline std::string getMM();
        inline std::string getDD();
    ...
    inline std::string Date::getYYYY()
    {
        return std::to_string(this->yyyy);
    }
    inline std::string Date::getMM()
    {
        return append_leading_zero(std::to_string(this->mm));
    }
    inline std::string Date::getDD()
    {
        return append_leading_zero(std::to_string(this->dd));
    }
    ...
    public:
        ...
        inline Result write_agencies_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_stops_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_routes_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_trips_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_stop_times_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_calendar_ISR(const std::string& gtfs_path) const;
        ...
        inline Result write_shapes_ISR(const std::string& gtfs_path) const;
    ...
    private:
        ...
        inline void write_agencies_ISR(std::ofstream& out) const;
        inline void write_routes(std::ofstream& out) const;
        inline void write_routes_ISR(std::ofstream& out) const;
        inline void write_shapes(std::ofstream& out) const;
        inline void write_shapes_ISR(std::ofstream& out) const;
        inline void write_trips(std::ofstream& out) const;
        inline void write_trips_ISR(std::ofstream& out) const;
        inline void write_stops(std::ofstream& out) const;
        inline void write_stops_ISR(std::ofstream& out) const;
        inline void write_stop_times(std::ofstream& out) const;
        inline void write_stop_times_ISR(std::ofstream& out) const;
        inline void write_calendar(std::ofstream& out) const;
        inline void write_calendar_ISR(std::ofstream& out) const;
    ...
    inline void Feed::updateStoptimes(int& update)
    {
        if (update != -1)
        {
            std::unordered_map<long long int, int> routesToAg;
            ...
            int countPerc = 0;//percentage for loading
            int size = this->stop_times.size();
            int check = 0;//check for loading
            int countIMirror=0;
            for (int i = 0; i < size; i++)
            {
                ...
                if (this->stop_times[i].stop_sequence != 1)//&&(routesToAg[stoll(stop_times[i].route_id)]!=ISRAEL_RAIL))
                {
                    ...
                    this->stop_times[i].departure_time = this->stop_times[i].arrival_time;
                }
            }
        }
    }
        ...
        const std::function<Result(const ParsedCsvRow& record)>& add_entity)
    {
        ...
        while ((res_row = parser.read_row(record)) != ResultCode::END_OF_FILE)
        {
            ...
            Result res = add_entity(record);//here
            if (filename == "trips.txt")
            {
                if ((this->trips.size() / (TRIPS_LINES/100) > i)&&(check ==0))
                {
                    if (i >= 99)
                    {
                        std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            {
                if ((this->routes.size() / (ROUTES_LINES / 100) > i)&&(check == 0))
                {
                    if (i >= 99)
                    {
                        std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            {
                if ((this->stops.size() / (STOPS_LINES / 100) > i)&&(check == 0))
                {
                    if (i >= 99)
                    {
                        std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            {
                if ((this->stop_times.size() / (STOPTIMES_LINES / 100) > i)&&(check == 0))
                {
                    if (i >= 99)
                    {
                        std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            {
                if ((this->shapes.size() / (SHAPES_LINES / 100) > i)&&(check == 0))
                {
                    if (i >= 99)
                    {
                        std::cout << "\r" << "Loading: Done" << std::endl;
    ...
    inline Result Feed::write_agencies_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_agencies_ISR(out); };
        return write_csv(gtfs_path, file_agency, write_agency_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_stops_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_stops_ISR(out); };
        return write_csv(gtfs_path, file_stops, write_stops_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_routes_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_routes_ISR(out); };
        return write_csv(gtfs_path, file_routes, write_routes_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_trips_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_trips_ISR(out); };
        return write_csv(gtfs_path, file_trips, write_trips_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_stop_times_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_stop_times_ISR(out); };
        return write_csv(gtfs_path, file_stop_times, write_stop_times_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_calendar_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_calendar_ISR(out); };
        return write_csv(gtfs_path, file_calendar, write_calendar_header_ISR, container_writer);
    }
    ...
    inline Result Feed::write_shapes_ISR(const std::string& gtfs_path) const
    {
        auto container_writer = [this](std::ofstream& out) { return this->write_shapes_ISR(out); };
        return write_csv(gtfs_path, file_shapes, write_shapes_header_ISR, container_writer);
    }
    ...
    inline void Feed::write_agencies_ISR(std::ofstream& out) const
    {
        for (const auto& agency : agencies)
        {
            std::vector<std::string> fields{ wrap(agency.agency_id),  wrap(agency.agency_name),
    ...
    inline void Feed::write_routes_ISR(std::ofstream& out) const
    {
        int count = 0;
        int perc = 0;
        int check = 0;
        ...
        {
            count++;
            if ((count / (size / 100) > perc) && (check == 0))
            {
                if (perc >= 99)
                {
                    std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            std::vector<std::string> fields{ wrap(route.route_id),
                                            ...
                                            wrap(route.route_long_name),
                                            wrap(route.route_desc),
                                            wrap(gtfs::RouteType::Bus),//writing all routes types as 3 (bus) because of accessibiliyt software limitations for now
                                            route.route_color};
            // TODO: handle new route fields.
            write_joined(out, std::move(fields));
        }
    ...
    inline void Feed::write_shapes_ISR(std::ofstream& out) const
    {
        int count = 0;
        int perc = 0;
        int check = 0;
    ...
    inline void Feed::write_trips_ISR(std::ofstream& out) const
    {
        int count = 0;
        int perc = 0;
        int check = 0;
    ...
    inline void Feed::write_stops_ISR(std::ofstream& out) const
    {
        int count = 0;
        int perc = 0;
        int check = 0;
        ...
        {
            count++;
            if ((count / (SHAPES_LINES / 100) > perc) && (check == 0))
            {
                if (perc >= 99)
                {
                    std::cout << "\r" << "Loading: Done" << std::endl;
            ...
            std::vector<std::string> fields{
                wrap(stop.stop_id),        wrap(stop.stop_code),    wrap(stop.stop_name),
                wrap(stop.stop_desc),      wrap(stop.stop_lat),     wrap(stop.stop_lon),
                wrap(stop.location_type),
                wrap(stop.parent_station),
                wrap(stop.zone_id) };
            write_joined(out, std::move(fields));
        }
    }
    ...
    inline void Feed::write_stop_times_ISR(std::ofstream& out) const
    {
        int count = 0;
        int perc = 0;
        int check = 0;
    ...
    inline void Feed::write_calendar_ISR(std::ofstream& out) const
    {
        for (const auto& item : calendar)
        {
            std::vector<std::string> fields{
                wrap(item.service_id),         wrap(item.sunday),       wrap(item.monday),   wrap(item.tuesday),
                wrap(item.wednesday),        wrap(item.thursday), wrap(item.friday),
                wrap(item.saturday),   item.start_date.get_raw_date(),
                item.end_date.get_raw_date() };
            write_joined(out, std::move(fields));
        }
    }
